...
print("FINAL V33 FIXED - GAP + CLEAR + RATE LIMIT FIX + HINDZINC HINDCOPPER")
...
def wait_for_valid_token():
    ...
    while True:
        ...
        if tok and is_token_valid(tok):
            ...
            return tok
        print("⏳ No valid token - waiting 60 sec for /upstox-login...")
        send_telegram_alert(f"⚠ <b>Upstox Token Missing!</b>\nLogin: https://ravialgo.onrender.com/upstox-login\nTime: {datetime.now(IST).strftime('%H:%M:%S')}")
        time.sleep(60)
...
def connect_sheets():
    ...
    while True:
        try:
            ...
            print(f"✅ Sheets connected! From: {weekly_from} To: {weekly_to}")
            return True
        except Exception as e:
            retry += 1
            print(f"❌ SHEET ERROR {retry}: {e} - retry 30 sec")
            time.sleep(30)
...
for attempt in range(5):
    try:
        ...
        with gzip.GzipFile(fileobj=io.BytesIO(r.content)) as gz:
            df = pd.read_csv(gz)
        print(f"✅ Instrument list: {len(df)} rows")
        break
    except Exception as e:
        print(f"Instrument fail {attempt+1}: {e}")
        time.sleep(10)
else:
    df = pd.DataFrame()
...
def get_candle(k, fro, to):
    ...
    try:
        ...
        if resp.status_code==200: return k, resp.json().get("data",{}).get("candles",[])
            ...
            print(f"429 Rate limit for {k} - sleep 2 sec")
            time.sleep(2)
    except: pass
    return k, []
...
with ThreadPoolExecutor(max_workers=3) as ex: # Reduced workers to avoid rate limit
...
def fetch_ltp(keys):
    ...
    try:
        ...
        if resp.status_code==200:
            ...
            print("LTP 429 - sleep 3 sec")
            time.sleep(3)
    except Exception as e:
        print(f"LTP ERR {e}")

print("Fetching LTP with delay...")
for i in range(0, len(all_keys), 10): # batch 10 only
    fetch_ltp(all_keys[i:i+10])
    time.sleep(0.8) # delay between batches
...
def build_sorted():
    ...
    for it in indices:
        ...
        volx_str = "N/A" if it.get("is_index") else f"{it['vol']/it['prev_vol']:.1f}X" if it["prev_vol"]>0 else "0.0X"
        vol_display = 0 if it.get("is_index") else it["vol"]
        prev_vol_display = 0 if it.get("is_index") else it["prev_vol"]
        status=get_status(it)
        if status and not it["break_time"]: it["break_time"]=datetime.now(IST).strftime("%H:%M:%S")
        rows.append([it["symbol"],it["pdh"],it["pdl"],it["wh"],it["wl"],it["ltp"],f"{it['change']:.2f}%",vol_display,prev_vol_display,volx_str,f"{dist:.1f}%",status,it["break_time"],datetime.now(IST).strftime("%H:%M:%S")])
        row_map.setdefault(it["symbol"], []).append(rnum)
        rnum+=1
    rows.append([""]); rnum+=1 # GAP - FIXED
    for sec_name in STRUCTURE.keys():
        ...
        for it in stocks:
            ...
            row_map.setdefault(it["symbol"], []).append(rnum)
            rnum+=1
        rows.append([""]); rnum+=1 # GAP AFTER EACH SECTOR - MAIN FIX
    return rows
...
def build_breakout_sheet():
    ...
    for sec_name in STRUCTURE.keys():
        ...
        if index_status in ["BREAKOUT","BREAKDOWN"]:
            ...
            volx_str = "N/A"
            rows.append([index_it["symbol"],index_it["pdh"],index_it["pdl"],index_it["wh"],index_it["wl"],index_it["ltp"],f"{index_it['change']:.2f}%",0,0,volx_str,f"{dist:.1f}%",index_status,index_it["break_time"],datetime.now(IST).strftime("%H:%M:%S"),"INDEX"])
...
def safe_sheet_update():
    for attempt in range(3):
        try:
            full = build_sorted()
            # FIX 1: Clear old data from A4 onwards to remove duplicate/stale rows
            try:
                sheet.batch_clear(["A4:Z5000"])
                print("✅ Sheet cleared A4:Z5000")
                time.sleep(1)
            except Exception as ce:
                print(f"Clear warning: {ce}")
            ...
            print(f"DONE {len(row_map)} rows - FIXED")
            return True
        except Exception as e:
...
def start_streamer_with_reconnect():
    while True:
        if not is_market_open():
            print(f"[{datetime.now(IST).strftime('%H:%M:%S IST')}] {get_market_status_msg()} - Sleep 60 sec...")
            time.sleep(60)
            continue
        try:
            print(f"[{datetime.now(IST).strftime('%H:%M:%S')}] Starting Upstox Streamer V33 FIXED")
            global UPSTOX_ACCESS_TOKEN
                            ...
                            if ltp:
                                ...
                                if new_status in ["BREAKOUT","BREAKDOWN"] and prev_status != new_status:
                                    ...
                                    if last_alert != new_status:
                                        ...
                                        msg = (f"{emoji} <b>{new_status} ALERT - {sym}</b> {emoji}\n\n💰 LTP: {instrument_data[ikey]['ltp']:.2f}\n📈 Change: {chg_val:.2f}%\n📊 WH: {instrument_data[ikey]['wh']:.2f} | WL: {instrument_data[ikey]['wl']:.2f}\n📦 Vol X: {volx:.1f}X | Dist: {dist:.1f}%\n⏰ Time: {instrument_data[ikey]['break_time']} IST\n🔍 Type: {'INDEX' if instrument_data[ikey]['is_index'] else 'STOCK'}")
            ...
            def on_open():
                print("✅ LIVE CONNECTED - V33 FIXED")
                send_telegram_alert("✅ <b>Ravi Algo LIVE CONNECTED - V33 FIXED</b>")
                                ...
                                for ikey, it in instrument_data.items():
                                    ...
                                    if sym in row_map:
                                        ...
                                        for rnum in rnums:
                                            ...
                                            volx_str = "N/A" if it.get("is_index") else f"{volx:.1f}X"
                                            status=get_status(it)
                                            batch.append({"range": f"F{rnum}:N{rnum}", "values": [[it["ltp"], f"{it['change']:.2f}%", it["vol"] if not it.get("is_index") else 0, it["prev_vol"] if not it.get("is_index") else 0, volx_str, f"{dist:.1f}%", status, it["break_time"], datetime.now(IST).strftime("%H:%M:%S")]]})
                                if batch:
                                    try:
            ...
            send_telegram_alert(f"⚠ Streamer crash: {str(e)[:100]}")
            time.sleep(15)
